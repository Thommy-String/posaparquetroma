...
// Categorie incollate (non adatte alla posa su pavimento esistente)
const GLUED_CATEGORIES = ['prefinito', 'prefinito-spina'];
...
export default function WorksCarousel({ title = 'Quanto è costato gli altri?', hideIncollati = false }) {
  // Filtra le categorie visibili: nasconde quelle incollate se richiesto
  const visibleCategories = useMemo(() => {
    if (!hideIncollati) return WORK_CATEGORIES;
    return WORK_CATEGORIES.filter(c => c.id === 'all' || !GLUED_CATEGORIES.includes(c.id));
  }, [hideIncollati]);
  ...
  const filteredWorks = useMemo(() => {
    ...
    // Nasconde i lavori incollati se richiesto
    if (hideIncollati) {
      list = list.filter(w => !GLUED_CATEGORIES.includes(w.category));
    }
  ...
  useEffect(() => {
    ...
    if (match) {
      const cat = match[1];
      if (WORK_CATEGORIES.some(c => c.id === cat) && (!hideIncollati || !GLUED_CATEGORIES.includes(cat))) {
