...
import certificazioniBadge from '../assets/images/italwood/spcitalwood/certificazioni/certificazioni-spc.png';
...
import spcPosaVideo from '../assets/videos/andreaPosaSPC.webm';
import spcPosaCardVideo from '../assets/videos/spcPosaRoma-2.webm';
...
// ═══════════════════════════════════
// ═══ LIGHTBOX (con animazioni fluide) ═══
// ═══════════════════════════════════
const Lightbox = ({ src, alt, onClose, closing }) => {
  ...
  const isActive = mounted && !closing;

  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center p-4 cursor-pointer"
      style={{
        backgroundColor: `rgba(0,0,0,${isActive ? 0.85 : 0})`,
      ...
      onClick={onClose}
    >
      {/* Close button — sempre visibile con sfondo scuro */}
      <button
        onClick={(e) => { e.stopPropagation(); onClose(); }}
        className="absolute top-4 right-4 z-10 w-10 h-10 flex items-center justify-center rounded-full bg-black/50 backdrop-blur-sm text-white/90 hover:bg-black/70 hover:text-white hover:scale-110 active:scale-90 border border-white/15 shadow-lg transition-all duration-200"
        style={{
      ...
      {/* Immagine con zoom-in all'apertura */}
      <img
        src={src}
        alt={alt}
...
const PriceCard = ({ totale, conPosa, sqm, displaySqm, selectedProduct, selectedColor, visible, prezzoPieno, prezzoSconto, prezzoPosa, risparmio, giorni, collapsed, battiscopaTipo, prezzoBattiscopaFornitura, prezzoBattiscopaPosa, costoTrasporto, conMaterassino, prezzoMaterassino, prezzoSmaltimento, showFullBreakdown, onToggleBreakdown, isSottoSoglia, costoFissoPosa, costoFissoBattiscopaPosa }) => {
  ...
  const handleWhatsApp = () => {
    ...
    track('cta_whatsapp_quiz', { source: 'quiz', price: totale });
    window.open(`https://wa.me/39${PHONE_NUMBER.replace(/[^0-9]/g, '')}?text=${msg}`, '_blank');
  };

  return (
    <>
      {/* Backdrop animato per chiudere cliccando fuori */}
      <div
        className={`fixed inset-0 z-30 transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] ${
          showFullBreakdown
            ? 'bg-black/30 opacity-100 pointer-events-auto'
            : 'bg-black/0 opacity-0 pointer-events-none'
        }`}
        onClick={() => onToggleBreakdown(false)}
      />
      <div className="fixed bottom-0 left-0 right-0 z-40 pointer-events-none px-4 pb-4 animate-[cardEntry_0.5s_cubic-bezier(0.34,1.56,0.64,1)_both]">
        <div className={`max-w-lg mx-auto w-full bg-white/95 backdrop-blur-xl rounded-2xl border border-gray-200/80 pointer-events-auto transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] ${
          showFullBreakdown
            ? 'shadow-[0_16px_48px_rgba(0,0,0,0.18)] border-gray-300/60'
          ...
          <div className="px-4 py-3.5">
            ...
            <div className="flex items-center gap-2.5 mb-3">
              ...
              {/* Compact total + CTA — always visible when breakdown is NOT open */}
              <div className={`flex items-center gap-2 shrink-0 transition-all duration-400 ease-[cubic-bezier(0.4,0,0.2,1)] ${!showFullBreakdown ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'}`}>
                <button onClick={handleWhatsApp}
                  className="shrink-0 inline-flex items-center gap-1 bg-white hover:bg-gray-50 text-green-600 border border-green-200 px-3 py-2 rounded-lg font-semibold text-xs shadow-sm hover:shadow-md transition-all duration-200 active:scale-95">
              ...
              {/* Close button — elegante, Apple-style, sempre all'estrema destra */}
              {showFullBreakdown && (
                <button onClick={() => onToggleBreakdown(false)} className="shrink-0 w-8 h-8 rounded-full bg-red-50 hover:bg-red-100 border border-red-200/70 flex items-center justify-center transition-all duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)] hover:rotate-90 active:scale-90 group">
            ...
            {!showFullBreakdown && (
              <div className="pb-2 space-y-2 animate-[textReveal_0.4s_cubic-bezier(0.4,0,0.2,1)_both]">
                ...
                <button onClick={() => onToggleBreakdown(true)} className="w-full py-3 text-center text-sm font-bold text-white bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 rounded-xl transition-all duration-200 shadow-md hover:shadow-lg flex items-center justify-center gap-2 active:scale-[0.97]">
            ...
            {/* Price grid — animato con grid-template-rows per smoothness Apple-like */}
            <div className="grid transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)]" style={{ gridTemplateRows: showFullBreakdown ? '1fr' : '0fr' }}>
              <div className="overflow-hidden">
                <div className="space-y-2 mb-3 min-h-0">
                  {/* ── FORNITURA ── */}
                  <div className="flex items-center justify-between text-sm" style={{ animation: showFullBreakdown ? `0.4s cubic-bezier(0.4,0,0.2,1) 0.05s both textReveal` : 'none' }}>
                  ...
                  <div className="flex items-center justify-between text-xs text-gray-400" style={{ animation: showFullBreakdown ? `0.4s cubic-bezier(0.4,0,0.2,1) 0.1s both textReveal` : 'none' }}>
                  ...
                  {battiscopaTipo && (
                    <>
                      <div className="flex items-center justify-between text-sm" style={{ animation: showFullBreakdown ? `0.4s cubic-bezier(0.4,0,0.2,1) 0.15s both textReveal` : 'none' }}>
                      ...
                      <div className="flex items-center justify-between text-xs text-gray-400" style={{ animation: showFullBreakdown ? `0.4s cubic-bezier(0.4,0,0.2,1) 0.2s both textReveal` : 'none' }}>
                  ...
                  {conMaterassino && (
                    <>
                      <div className="flex items-center justify-between text-sm" style={{ animation: showFullBreakdown ? `0.4s cubic-bezier(0.4,0,0.2,1) 0.25s both textReveal` : 'none' }}>
                        <span className="text-gray-500">Materassino per Spina</span>
                      ...
                      <div className="flex items-center justify-between text-xs text-gray-400" style={{ animation: showFullBreakdown ? `0.4s cubic-bezier(0.4,0,0.2,1) 0.3s both textReveal` : 'none' }}>
                  ...
                  <div className="border-t border-gray-100 pt-2 flex items-center justify-between" style={{ animation: showFullBreakdown ? `0.4s cubic-bezier(0.4,0,0.2,1) 0.35s both textReveal` : 'none' }}>
                  ...
                  {conPosa && (
                    <div style={{ animation: showFullBreakdown ? `0.4s cubic-bezier(0.4,0,0.2,1) 0.4s both textReveal` : 'none' }}>
                        ...
                        {isSottoSoglia ? (
                          <>
                            <div className="flex items-center justify-between text-sm">
                              ...
                              <span className="font-semibold text-gray-800 whitespace-nowrap"><AnimatedPrice value={costoFissoPosa} className="text-sm" /></span>
                            </div>
                            <div className="flex items-center justify-between text-xs text-gray-400">
                              <span>Prezzo fisso ({'<'} 40 mq)</span>
                              <span className="font-medium">€{costoFissoPosa}</span>
                            </div>
                            {battiscopaTipo && (
                              <>
                                <div className="flex items-center justify-between text-sm">
                                  <span className="text-gray-500">Posa battiscopa</span>
                                  <span className="font-semibold text-gray-800 whitespace-nowrap"><AnimatedPrice value={costoFissoBattiscopaPosa} className="text-sm" /></span>
                                </div>
                                <div className="flex items-center justify-between text-xs text-gray-400">
                                  <span>Prezzo fisso ({'<'} 40 mq)</span>
                                  <span className="font-medium">€{costoFissoBattiscopaPosa}</span>
                                </div>
                              </>
                            )}
                            {/* Notion-style yellow note */}
                            <div className="mt-2 px-3 py-2 rounded-lg bg-amber-50 border border-amber-200 text-[11px] leading-relaxed">
                              <p className="text-amber-800 font-semibold">📐 Per superfici inferiori a 40 mq applichiamo un costo fisso per la posa</p>
                              <p className="text-amber-700 mt-0.5">Posa SPC: <strong>€{costoFissoPosa}</strong> · Posa battiscopa: <strong>€{costoFissoBattiscopaPosa}</strong></p>
                            </div>
                          </>
                        ) : (
                          <>
                  ...
                  {/* ── COSTI AGGIUNTIVI ── */}
                  <div style={{ animation: showFullBreakdown ? `0.4s cubic-bezier(0.4,0,0.2,1) 0.45s both textReveal` : 'none' }}>
                    <div className="mt-3 pt-2 border-t border-gray-100">
                  ...
                  {/* ── TOTALE FINALE ── */}
                  {conPosa && (
                    <div className="mt-3 pt-2 border-t border-green-100 flex items-center justify-between" style={{ animation: showFullBreakdown ? `0.4s cubic-bezier(0.4,0,0.2,1) 0.5s both textReveal` : 'none' }}>
                  ...
                  {conPosa && (
                    <div className="flex items-center justify-between text-[11px] bg-green-50 -mx-4 px-4 py-2 rounded-lg" style={{ animation: showFullBreakdown ? `0.4s cubic-bezier(0.4,0,0.2,1) 0.55s both textReveal` : 'none' }}>
                  ...
                  {!conPosa && (
                    <div className="mt-3 pt-2 border-t border-gray-100 flex items-center justify-between" style={{ animation: showFullBreakdown ? `0.4s cubic-bezier(0.4,0,0.2,1) 0.5s both textReveal` : 'none' }}>
                      <span className="text-sm font-bold text-gray-700">Totale</span>
                      <AnimatedPrice value={totale} className="text-lg font-bold text-gray-900" />
                    </div>
                  )}
                ...
                {/* ── CTA SALVA PREVENTIVO SU WHATSAPP ── */}
                <div className="px-1 pt-1 border-t border-gray-100" style={{ animation: showFullBreakdown ? `0.5s cubic-bezier(0.34,1.56,0.64,1) 0.6s both textReveal` : 'none' }}>
                  <button
                    onClick={handleWhatsApp}
                    className="w-full inline-flex items-center justify-center gap-2.5 py-4 px-4 rounded-xl bg-white hover:bg-gray-50 border border-green-400 text-gray-700 font-semibold text-sm transition-all duration-200 hover:border-green-300 hover:text-green-700 hover:shadow-sm active:scale-[0.98] group"
                  >
                    <svg viewBox="0 0 24 24" width="18" height="18" fill="none" aria-hidden="true" className="text-green-500 shrink-0 group-hover:text-green-600 transition-colors">
                      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" fill="currentColor"/>
                    </svg>
                    <span>Salva il preventivo su Whatsapp</span>
                  </button>
                  <p className="text-[11px] text-gray-400 text-center mt-2 leading-tight">🔒 non condividiamo il tuo numero con nessuno</p>
                </div>
              </div>
            </div>
          </div>
...
const WorkCarousel = () => {
  ...
  const totalWidth = spcWorks.length * cardW;

  // Auto-scroll animation — directly manipulates DOM for zero-lag performance
  useEffect(() => {
    ...
    const step = () => {
      if (!pausedRef.current && !isDraggingRef.current) {
        translateXRef.current -= autoScrollSpeed;
  ...
  const handleDragMove = (clientX) => {
    ...
    const track = trackRef.current;
    if (track) {
      track.style.transform = `translateX(${translateXRef.current + delta}px)`;
    }
  };
  ...
  const handleDragEnd = () => {
    ...
    // Apply the drag offset to the permanent offset
    translateXRef.current += finalDragOffset;
    // Clamp: keep within bounds for smooth loop
    if (translateXRef.current > 0) {
      translateXRef.current = -totalWidth + (translateXRef.current % totalWidth);
    }
    if (Math.abs(translateXRef.current) >= totalWidth * 2) {
      translateXRef.current = -totalWidth + (Math.abs(translateXRef.current) % totalWidth);
    }
            ...
            <div key={idx} className="min-w-[220px] w-[220px] shrink-0 bg-white rounded-xl border border-gray-100 overflow-hidden shadow-sm">
              ...
              <div className="relative h-28 overflow-hidden bg-gray-100">
                ...
                <div className="absolute top-1.5 right-1.5 bg-white/90 backdrop-blur-sm rounded-full px-1.5 py-0.5 flex items-center gap-1 shadow-sm">
                  ...
                  <span className="text-[8px] font-bold text-gray-500">Verificato da Google</span>
                </div>
...
const Calculator = () => {
  ...
  const costoFissoPosa = 700; // €700 fissi per posa SPC < 40 mq
  const costoFissoBattiscopaPosa = 300; // €300 fissi per posa battiscopa < 40 mq
  const isSottoSoglia = displaySqm < 40;
  ...
  const totBattiscopaPosa = conPosa ? (isSottoSoglia ? costoFissoBattiscopaPosa : prezzoBattiscopaPosa * displaySqm) : 0;
  ...
  return (
    <div className="w-full max-w-lg mx-auto" ref={calculatorRef}>
      <PriceCard
        ...
        isSottoSoglia={isSottoSoglia} costoFissoPosa={costoFissoPosa} costoFissoBattiscopaPosa={costoFissoBattiscopaPosa}
      />

      {/* FOTO PRODOTTO / VIDEO ANTEPRIMA */}
      <div className="relative mb-4">
        <div
          className="relative rounded-2xl overflow-hidden bg-gray-100 shadow-lg cursor-pointer group"
          ...
          {colorSelected ? (
            <>
              <img
                ...
                alt={selectedColor.code}
                className="w-full h-64 md:h-80 object-cover transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] group-hover:scale-[1.02]"
              />
              <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors duration-300" />
              {/* Overlay con nome e prezzo */}
              <div className="absolute inset-x-0 bottom-0 px-4 py-3 bg-gradient-to-t from-black/80 via-black/40 to-transparent">
                <div className="min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="text-lg font-bold text-white drop-shadow-sm">{selectedColor.code}</p>
                    <span className="text-base font-black text-white shrink-0">€{conPosa ? prezzoSconto : prezzoPieno}<span className="text-[11px] font-semibold text-white/60">/mq</span></span>
                    {conPosa && <span className="text-sm text-red-300 line-through font-medium shrink-0">€{prezzoPieno}/mq</span>}
                  </div>
                  <p className="text-xs text-white/70 mt-0.5">{selectedProduct.name} · {selectedProduct.desc}</p>
                </div>
              </div>
            </>
          ) : (
            <>
              ...
              {/* Freccia in giù + "Scegli il colore" */}
              <div className="absolute bottom-1 left-1/2 -translate-x-1/2 flex flex-col items-center gap-1">
                <span className="text-[11px] font-bold text-white drop-shadow-sm tracking-wider">Scegli il colore</span>
                <svg className="w-6 h-6 text-white animate-bounce" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
      ...
      {/* BADGE CERTIFICAZIONI — sempre visibile, larghezza piena su mobile */}
      <div className="flex justify-center mb-5">
        <div className="bg-white/90 backdrop-blur-sm rounded-xl shadow-sm overflow-hidden w-full sm:w-auto">
          <img
            src={certificazioniBadge}
            alt="Certificazioni SPC Italwood"
            className="w-full sm:w-48 h-auto object-contain"
          />
        </div>
      </div>

      {lightboxOpen && <Lightbox src={selectedColor.img} alt={selectedColor.code} onClose={handleCloseLightbox} closing={lightboxClosing} />}
              ...
              <button key={p.id} onClick={() => handleProductChange(p)}
                ...
                <div className="mb-3">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {p.id === 'classico' && (
                      <span className="inline-flex items-center bg-orange-100 text-orange-800 text-[10px] font-bold px-2 py-0.5 rounded-md border border-amber-300/60 leading-none">
                        Il più scelto
                      </span>
                    )}

                    {p.id === 'spina' && (
                      <span className="inline-flex items-center bg-green-100 text-green-800 text-[10px] font-bold px-2 py-0.5 rounded-md border border-green-300/60 leading-none">
                        Il più desiderato
                      </span>
                    )}
                  </div>
      ...
      {/* SLIDER mq — sezione evidenziata */}
      <div className="mb-8">
        <div className="rounded-2xl border-2 border-green-200 bg-gradient-to-b from-green-50/80 to-white px-5 py-5 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-lg bg-green-100 flex items-center justify-center shrink-0">
                <Maximize2 className="w-4 h-4 text-green-600" strokeWidth={2.5} />
              </div>
              <div>
                <span className="text-sm font-bold text-gray-800">Metri quadri</span>
                <p className="text-[10px] text-gray-400 font-medium leading-tight">Quanto è grande la superficie?</p>
              </div>
            </div>
            <div className="text-right shrink-0">
              <span className="text-3xl font-black text-gray-900 leading-none">{sqm}</span>
              <span className="text-sm font-bold text-green-600 ml-1">mq</span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between mt-2 px-1">
            <span className="text-[10px] font-semibold text-gray-400">10 mq</span>
            <span className="text-[10px] font-semibold text-gray-400">200 mq</span>
          </div>
        </div>

        {/* ⚠️ AVVISO SOGLIA 40mq — animato con comparsa/scomparsa fluida */}
        <div
          className="grid transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)]"
          style={{ gridTemplateRows: isSottoSoglia ? '1fr' : '0fr', opacity: isSottoSoglia ? 1 : 0, marginTop: isSottoSoglia ? 12 : 0 }}
        >
          <div className="overflow-hidden">
            <div
              className={`px-4 py-3 rounded-xl border transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] ${
                isSottoSoglia
                  ? 'bg-amber-50 border-amber-300 shadow-sm translate-y-0'
                  : 'bg-amber-50 border-amber-300 translate-y-2'
              }`}
              style={{ transform: isSottoSoglia ? 'translateY(0) scale(1)' : 'translateY(8px) scale(0.98)' }}
            >
              <div className="flex items-start gap-2.5">
                <div className="shrink-0 w-7 h-7 rounded-full bg-amber-100 border border-amber-300 flex items-center justify-center">
                  <span className="text-sm">📐</span>
                </div>
                <div className="min-w-0">
                  <p className="text-[12px] font-bold text-amber-800 leading-snug">
                    Sotto i 40 mq la posa ha un costo fisso
                  </p>
                  <p className="text-[11px] text-amber-700 mt-0.5 leading-relaxed">
                    Posa pavimento <strong>€{costoFissoPosa}</strong> + posa battiscopa <strong>€{costoFissoBattiscopaPosa}</strong> — nessun costo a metro quadro per la posa.
                  </p>
                </div>
              </div>
            </div>
      ...
      <div className={`mb-6 rounded-2xl overflow-hidden transition-all duration-300 border-2 ${
        ...
        <div onClick={handleTogglePosa} className="cursor-pointer">
          ...
          <div className="relative w-full h-48 md:h-64 overflow-hidden bg-gray-100">
            ...
            {/* Prezzo posa overlay sul video */}
            <div className="absolute bottom-3 left-3">
              <div className={`backdrop-blur-sm rounded-xl px-3.5 py-2 shadow-lg border transition-all duration-300 ${
                conPosa 
                  ? 'bg-green-500/25 border-green-400/40' 
                  : 'bg-black/50 border-white/10'
              }`}>
                <div className="flex items-baseline gap-1.5">
                  <span className={`text-[10px] font-bold uppercase tracking-wider ${conPosa ? 'text-green-200' : 'text-white/50'}`}>
                    Posa completa
                  </span>
                </div>
                <div className="flex items-baseline gap-1 mt-0.5">
                  <span className={`text-xl font-black ${conPosa ? 'text-white' : 'text-white/70'}`}>€{prezzoPosa}</span>
                  <span className={`text-xs font-semibold ${conPosa ? 'text-white/70' : 'text-white/40'}`}>/mq</span>
                </div>
              </div>
            </div>
          </div>
                ...
                <div className="flex items-baseline gap-1.5 mt-0.5 flex-wrap">
                  {conPosa ? (
                    <span className="text-[11px] text-gray-600">
                      ...
                      sconto <span className="font-bold text-green-700">-40%</span>
                    </span>
                  ) : (
    ...
    <>
      ...
      <section className="bg-white overflow-x-hidden">
        <div className="max-w-lg mx-auto px-4 py-8 md:py-12">
          ...
          {/* TITOLO */}
          <h1 className="text-center my-6 max-w-3xl mx-auto ">
            <span className="block text-2xl md:text-5xl font-extrabold text-gray-900 leading-[1.15] tracking-tight">
              Nuovo pavimento <span className='text-green-500'>SPC a Roma</span> <br /> in 48 ore <span className='text-red-400'>senza demolizioni</span><br />
            </span>
          </h1>


          {/* 3 BENEFIT CHECK sotto il titolo */}
          <div className="flex flex-wrap items-center justify-center gap-x-2 gap-y-1.5 mb-6">
            {[
              { text: 'Garanzia 15 anni', icon: '🏆' },
              { text: 'Sul vecchio pavimento', icon: '⚡' },
              { text: 'Prezzi fissi', icon: '⚡' },
            ].map(({ text, icon }) => (
          ...
          <div className="bg-green-50 border border-green-200 rounded-xl px-4 py-2.5 text-center mb-4">
            <p className="text-sm font-bold text-green-800">
              <span className="text-green-500 font-black tracking-tight">🎉 Sconto -40% sul pavimento</span> se aggiungi la posa
            </p>
          </div>
